...
async function handleSendOtp() {
  ...
  if (users.some(u => u.email === email)) {
    showToast('<i class="fa-solid fa-triangle-exclamation"></i> Email address already registered!', 'error');
    return;
  }
  ...
  try {
    // బ్యాకెండ్ సర్వర్‌కు రిక్వెస్ట్ పంపడం
...
async function handleSignup(event) {
  ...
  try {
    const response = await fetch('http://localhost:3000/verify-and-register', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // ఫ్రంటెండ్‌లో కూడా యూజర్‌ను సేవ్ చేయడం
    const users = JSON.parse(localStorage.getItem('shopUsers')) || [];
...
function handleLogin(event) {
  ...
  if (!user) {
    showToast('<i class="fa-solid fa-triangle-exclamation"></i> Invalid email or password.', 'error');
    return;
  }
...
async function handleForgotPasswordSendOtp(event) {
  ...
  const users = JSON.parse(localStorage.getItem('shopUsers')) || [];
  if (!users.some(u => u.email === email)) {
    showToast('<i class="fa-solid fa-triangle-exclamation"></i> No account found with this email.', 'error');
    return;
  }
...
async function handleProfileUpdate(event) {
  ...
  if (newEmail !== currentUser.email && users.some(u => u.email === newEmail)) {
    showToast('<i class="fa-solid fa-triangle-exclamation"></i> Email address already taken!', 'error');
    return;
  }
...
function renderOrders() {
  ...
  const userOrders = allOrders.filter(o => o.email === currentUser.email).reverse();
...
function submitReturnRequest(event) {
  ...
  if (orderIdx !== -1) {
    allOrders[orderIdx].status = 'return requested';
    allOrders[orderIdx].returnReason = reason;
    allOrders[orderIdx].returnComments = comments;
    localStorage.setItem('shopOrders', JSON.stringify(allOrders));

    showToast(`<i class="fa-solid fa-circle-check"></i> Return request submitted for order #${orderId}`, 'success');
    closeReturnModal();
    renderOrders();
  } else {
    showToast('<i class="fa-solid fa-triangle-exclamation"></i> Order not found!', 'error');
  }
}
...
window.addEventListener('message', (event) => {
  ...
  if (event.origin.startsWith('http://localhost:3000')) {
    // The event.data will be the user object stringified from the server
    ...
    if (!existingUser) {
      ...
      const newUser = { name: user.name, email: user.email, password: 'social_login' };
      users.push(newUser);
      localStorage.setItem('shopUsers', JSON.stringify(users));
    ...
    // Log the user in
    currentUser = { name: existingUser.name, email: existingUser.email, avatar: user.avatar };
    localStorage.setItem('currentUser', JSON.stringify(currentUser));
    closeAuthModal();
    updateAuthUI();
